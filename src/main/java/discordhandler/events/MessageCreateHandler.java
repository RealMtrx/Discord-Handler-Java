...
import discordhandler.core.PrefixCommandWebhook;
...
public class MessageCreateHandler extends ListenerAdapter {
    ...
    public MessageCreateHandler(Bot bot) {
        this.bot = bot;
    }
    ...
    public void onMessageReceived(MessageReceivedEvent event) {
        ...
        try {
            ...
            PrefixCommandWebhook.sendUsage(
                    userId, event.getAuthor().getName(), cmd.name,
                    event.getGuild().getName(), event.getAuthor().getAvatarUrl()
            );
        } catch (Exception e) {
            System.out.println("  \u274c Error in prefix command " + cmd.name + ": " + e.getMessage());
            event.getChannel().sendMessage("\u274c **Error:** " + e.getMessage()).queue();

            PrefixCommandWebhook.sendError(
                    userId, event.getAuthor().getName(), cmd.name,
                    event.getGuild().getName(), e.getMessage()
            );
        }
    }
}
